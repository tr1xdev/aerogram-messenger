...
func (r *mutationResolver) MarkDialogAsRead(ctx context.Context, chatID string) (bool, error) {
	...
	log.Printf("[RESOLVER] MarkDialogAsRead: user %s, chat %s", authID, chatID)
	...
	if err != nil {
		log.Printf("[RESOLVER] No messages found in chat %s to mark as read", chatID)
		return true, nil
	}
...
func (r *subscriptionResolver) MessageAdded(ctx context.Context, chatID string) (<-chan *model.Message, error) {
	...
	log.Printf("[SUBSCRIPTION] Initializing MessageAdded for chat: %s", chatID)
	...
	go func() {
		...
		for {
			select {
			case <-ctx.Done():
				log.Printf("[SUBSCRIPTION] Context done for chat: %s", chatID)
				return
			case msg, ok := <-pubsub.Channel():
				if !ok {
					log.Printf("[SUBSCRIPTION] Redis channel closed for chat: %s", chatID)
					return
				}

				log.Printf("[SUBSCRIPTION] New message received from Redis on channel %s", channel)

				var pbMsg messagesv1.Message
				if err := unmarshaller.Unmarshal([]byte(msg.Payload), &pbMsg); err != nil {
					log.Printf("[SUBSCRIPTION] Error unmarshalling proto message: %v", err)
					continue
				}

				m := helpers.MapMessageToModel(&pbMsg)
				if m == nil {
					log.Printf("[SUBSCRIPTION] Failed to map message to model")
					continue
				}

				select {
				case out <- m:
					log.Printf("[SUBSCRIPTION] Message successfully sent to GraphQL client (ID: %s)", m.ID)
				case <-ctx.Done():
					return
				}
			}
...
func (r *subscriptionResolver) MessageUpdated(ctx context.Context, chatID string) (<-chan *model.Message, error) {
	out := make(chan *model.Message, 1)
	pubsub := r.RedisClient.Subscribe(ctx, "chat:"+chatID+":updated")

	log.Printf("[SUBSCRIPTION] Initializing MessageUpdated for chat: %s", chatID)
...
func (r *subscriptionResolver) MessageDeleted(ctx context.Context, chatID string) (<-chan string, error) {
	...
	pubsub := r.RedisClient.Subscribe(ctx, "chat:"+chatID+":deleted")

	log.Printf("[SUBSCRIPTION] Initializing MessageDeleted for chat: %s", chatID)
...
func (r *subscriptionResolver) DialogRead(ctx context.Context, chatID string) (<-chan *model.ReadPayload, error) {
	...
	log.Printf("[SUBSCRIPTION] Initializing DialogRead for chat: %s (User: %s)", chatID, authID)
