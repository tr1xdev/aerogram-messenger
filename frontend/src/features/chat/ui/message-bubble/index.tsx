...
import { ImageAttachmentItem, FileAttachmentItem } from "./ui/attachment-list";
...
export const MessageBubble = memo(function MessageBubble({
  ...
  const { images, files, mappedAllFiles } = useMemo(() => {
    const imgs: Attachment[] = [];
    const fls: Attachment[] = [];
    const mapped: Array<{
      url: string;
      fileName: string;
    ...
    attachments.forEach((a) => {
      ...
      mapped.push({ url, fileName, fileSize, contentType });
    });

    return { images: imgs, files: fls, mappedAllFiles: mapped };
  }, [attachments]);
